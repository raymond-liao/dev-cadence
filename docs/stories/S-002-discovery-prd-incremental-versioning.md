...
- 用户不同意迁移时，在原路径继续增量更新，并在最终确认摘要中列出实际权威文档路径。
- 一份综合文档同时包含 PRD 和 Business Architecture 内容时，不自动拆分；由用户选择保持单文件或拆分为两份文档。
- 保持综合单文件时，分别维护 PRD 与 Business Architecture 职责版本，Change Log 标明受影响职责；拆分后各文件独立维护 Version 和 Change Log。
...
- 最终确认摘要列出本次产品设计基线实际使用的文档路径和版本组合；Discovery 不创建 manifest 或阶段记录。
- 增量模式先读取权威资产并在会话中形成完整 proposed revised baseline 与变更摘要；用户统一确认前，磁盘权威资产和相关支撑资产保持原样，反馈或拒绝只更新会话 proposal，且不创建草稿过程文件。
- 用户统一确认后，原子应用受影响的权威内容、职责版本、Change Log、迁移或拆分、引用更新和相关支撑资产维护；不得把部分写入结果声明为新基线。
...
1. `discovery` 可以读取现有 PRD 和 Business Architecture 内容与版本，在会话中形成完整 proposed revised baseline 与变更摘要，并只在用户统一确认后原子应用协调的增量变更。
...
6. 综合产品文档只有在用户明确同意后才拆分；保留单文件时同一路径分别报告并维护 PRD Version 与 Business Architecture Version（或等价职责版本），Change Log 标明受影响职责。
7. PRD 和 Business Architecture 职责独立管理版本；产品目标、范围、成功标准、业务角色、业务域、流程、对象、状态、规则或约束发生实质变化时，只递增受影响文档或综合文档职责的版本并记录变化原因；拆分后各文件独立维护 Version。
...
11. 最终确认摘要列出本次确认的实际文档路径和职责版本组合；Discovery 不创建 manifest、阶段记录或草稿 proposal 文件，更新后的完整组合得到用户确认前不写入权威资产、不执行相关支撑资产维护，也不成为后续规划的正式基线；反馈或拒绝只更新会话 proposal。
