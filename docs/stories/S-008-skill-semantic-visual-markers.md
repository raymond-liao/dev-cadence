...
7. `using-dev-cadence` 要求 workflow 在创建或更新 Dev Cadence 管理的 Markdown 前读取共享规范，但不复制完整语义表。
8. 构建和安装包包含 `document-conventions`，契约测试验证共享语义表、入口接入和代表性关键区块，同时不锁死全部自然语言或要求无差别装饰。
