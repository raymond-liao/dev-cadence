...
- 当用户明确要求按 `docs/backlog.md` 的“当前可并行实施表”继续实施时，由 `using-dev-cadence` 在路由下游 workflow 前选择下一张符合顺序、依赖和入口资格的工作项。
...
7. 用户明确要求按“当前可并行实施表”继续实施时，入口先选择下一张符合条件的卡片，再同步更新卡片与 Backlog 为 `In Progress`，最后才切换任务分支、创建 worktree 或进入下游 workflow。
