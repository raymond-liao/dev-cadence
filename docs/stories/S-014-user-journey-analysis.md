...
|---:|---|---|---|---|
| 1 | legacy: recorded-at precision unknown; original 2026-07-14 | legacy: recorded-by unknown | 创建 User Journey 分析卡片。 | 将 User Journey 作为可选产品分析能力保留到后续设计。 |
| 2 | legacy: recorded-at precision unknown; original 2026-07-15 | legacy: recorded-by unknown | 将卡片重定义为 Discovery 内的 User Journey 与 Feature 基线增强，并更新优先级、依赖和验收范围。 | 最新 Discovery 流程已确认 Journey 是 PRD 与 Business Architecture 的前置权威资产，Feature 必须由 Discovery 统一创建和维护。 |
| 2 | legacy: recorded-at precision unknown; original 2026-07-16 | legacy: recorded-by unknown | 三资产两门契约已实现并验证，完成 User Journey 与 Feature 基线交付。 | Discovery 源码、分发包、入口路由、公开说明和契约验证已同步通过。 Legacy migration: original Version 3; normalized to Version 2. |
| 2 | legacy: recorded-at precision unknown; original 2026-07-16 | legacy: recorded-by unknown | 实施与系统验证已完成，等待 Business Acceptance 后再进入 Done。 | Delivery Workflow 规则要求 Business Acceptance 不能由实施授权替代。 Legacy migration: original Version 4; normalized to Version 2. |
| 2 | legacy: recorded-at precision unknown; original 2026-07-16 | legacy: recorded-by unknown | 记录 Business Acceptance 并将状态更新为 Done。 | 用户选择 `1. Accept`，S-014 交付结果已验收。 Legacy migration: original Version 5; normalized to Version 2. |
| 2 | 2026-07-19T13:07:24+0800 | Raymond Liao <raymond-liao@outlook.com> | Normalized legacy status and delivery events to reuse the active definition Version. | Old current 5 -> new current 2; original row versions 1,2,3,4,5 -> normalized row versions 1,2,2,2,2. |
