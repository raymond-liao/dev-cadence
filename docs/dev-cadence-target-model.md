...
| 启动方式 | session-start 注入 `using-superpowers` | session-start 注入 `using-dev-cadence` |
...
- 在 session-start 注入 Dev Cadence 基本规则；
...
session-start 已注入 `using-dev-cadence`。
...
4. 实现 `using-dev-cadence` 和 session-start bootstrap。
