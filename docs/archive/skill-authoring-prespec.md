...
- 普通 feature、bugfix、review、refactor、research 和 incident 工作由 `using-dev-cadence` 路由到 `cadence-clarify`、`cadence-plan`、`cadence-executing-plans`、`cadence-subagent-development`、`cadence-dispatch-parallel`、`cadence-tdd`、`cadence-debug`、`cadence-request-code-review`、`cadence-code-review` 和 `cadence-verify`。
