...
Backlog 流程负责把实施工作可靠地接入 Dev Cadence。它创建或接收 Story、Task、Bug 卡片，校验卡片契约，维护 `docs/delivery/backlog.md`，并把满足条件的工作项交给单卡分析或 Delivery Workflow。
...
如果不合规来源已经位于 `docs/delivery/stories/`、`docs/delivery/tasks/` 或 `docs/delivery/bugs/` 下，Backlog 仍保持该文件原状且不登记它；新建的 Dev Cadence 卡片使用全新未占用 ID 和路径，避免覆盖、重命名或身份冲突。
...
- `docs/delivery/backlog.md`：Backlog 结构、生命周期分区和待处理建议顺序。
- `docs/delivery/stories/**`、`docs/delivery/tasks/**`、`docs/delivery/bugs/**`：工作项卡片。
