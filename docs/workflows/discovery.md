...
前四个阶段共同形成一套产品设计，并只作为会话内工作方法，不创建 manifest 或阶段记录。首次和增量模式都通过最后一次确认同时覆盖完整产品设计组合；确认前不得声称基线已完成。
...
- PRD 与 Business Architecture 独立管理版本；只有受影响文档发生实质变化时才递增，拼写、排版、路径、文件名和链接变更不升版。
...
- 扫描仓库根目录和正常项目文档目录，按内容职责识别 PRD、Business Architecture 和综合产品文档；排除 `.dev-cadence/`、`dist/`、`build/`、`vendor/`、`node_modules/` 和 `.git/`。
...
- 最终确认摘要列出实际文档路径、前后版本、未升版原因和潜在工作项影响；工作项调整交给 `work-item-planning`。
- 增量更新先读取当前权威内容和版本，在会话中形成完整 proposed revised baseline 与变更摘要；确认前磁盘上的权威资产保持原样，用户反馈或拒绝只更新会话 proposal，不创建草稿文件或过程记录。
- 用户统一确认后，才原子写入受影响的 PRD 与 Business Architecture 职责、递增相应版本、追加 Change Log，并执行已确认的迁移、拆分、引用更新和 Registry 等支撑资产维护。
...
- 首次模式的修改反馈更新当前草稿；增量模式的反馈只更新会话 proposal 并重新展示完整 proposed baseline。用户拒绝时不修改既有权威资产、不创建额外拒绝记录，也不声称基线已确认。
