...
| [x] | `cadence-clarify` | `brainstorming` | ❌ | 已完成 | 已同步 hard gate、just-in-time visual companion、one-question flow、2-3 approaches、design approval、spec self-review、Human artifact review 和 planning handoff；仍需按文件级结构和行为触发力再次确认。 |
...
| [x] | `cadence-debug` | `systematic-debugging` | ❌ | 已完成 | 已补 root-cause-first hard rule、四阶段调试、假设验证、失败后升级、human correction signals、common rationalizations、debug references 和污染定位脚本；仍需按文件级结构和行为触发力再次确认。 |
| [x] | `cadence-request-review` | `requesting-code-review` | ❌ | 已完成 | 已补 review 触发时机、精确上下文、只读 reviewer、reviewer prompts、severity、明确 verdict、spec compliance 和 code quality 两阶段 review；仍需按文件级结构和行为触发力再次确认。 |
| [x] | `cadence-review` | `receiving-code-review` | ❌ | 已完成 | 已补 verify-before-implement、禁止表演式同意、unclear feedback 停手、source-specific handling、pushback、逐项处理、GitHub thread reply 和 re-review handoff；仍需按文件级结构和行为触发力再次确认。 |
