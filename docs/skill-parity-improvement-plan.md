...
因此，像此前 `cadence-debug` 把大量只属于 debugging 的方法论拆进全局 reference 或远离 Skill-local 资源的情况，需要重新评估：
...
| [x] | `cadence-research` | 无直接对应 | N/A | 已完成 | 已完成 Dev Cadence 自有 research-spike 内部一致性 review；强化 research boundary、allowed sources、evidence rules、option analysis、recommendation confidence、Human Gate triggers、artifact-ready research-report handoff、stop conditions 和 no-implementation boundary；改用 gate-relevant observations / required Human decisions handoff，不让 Research Skill 拥有 gate status 或 persistent record 写入职责。 |
| [x] | `cadence-sync` | 无直接对应 | N/A | 已完成 | 已完成 Dev Cadence 自有 repo contract/runtime reconcile 边界 review；明确 initial bootstrap 不由缺失的 target-repo Skill 执行，Skill 只在 Dev Cadence 已由 embedded runtime、source checkout、global plugin 或显式 bundle/source 可用后检查/修复/诊断/对齐；不发现 upstream update，只有 Human/controller 提供 `expected_bundle_path`、`expected_source_path` 或 `expected_version` 时才比较/对齐 runtime；无 comparison target 时只报告 local runtime status / upstream freshness unknown；保留 thin contract 脚本边界、允许写入范围、local overlay/legacy `.ai/` 保留、drift 分类、sync report 和 Supervisor handoff；明确返回 repository contract status / Human decisions，不使用 generic gate status handoff。 |
...
- [x] N/A `cadence-research`
    - Dev Cadence 自有。
    - 产出：已强化 research-spike 与 workflow、artifact、Human decision 边界一致性；研究输出只作为 decision input，不批准 implementation；persistent artifacts 使用 artifact-ready handoff，不直接写入 records、Harness evidence、gate status 或 acceptance records；已补 `check-discipline-routes.mjs` 覆盖 research boundary、evidence rules、recommendation gate、stop conditions 和 responsibility boundary。

- [x] N/A `cadence-sync`
    - Dev Cadence 自有。
    - 产出：已强化 repo contract 操作边界、目标仓库 runtime 设计和同步脚本一致性；明确 initial bootstrap 不由 target repo 内尚不存在的 Skill 执行；`cadence-sync` 不发现 upstream update，只在外部提供 `expected_bundle_path`、`expected_source_path` 或 `expected_version` 时做 reconcile；无 comparison target 时只能报告 local runtime status 和 `upstream_freshness: unknown`；`sync-repo-contract.mjs` 只用于 thin contract；保留 local overlays 和 legacy `.ai/`，禁止触碰 product files / task-specific specs；已补 `check-discipline-routes.mjs` 覆盖 bootstrap boundary、comparison target requirement、runtime reconcile path、write boundary、drift classification、sync report 和 handoff wording。
