...
源码仓库本身也不长期追踪运行过程目录。`specs/` 是 Dev Cadence 在目标仓库或本地 dry run 中生成的任务 artifact；`research/` 是临时探索工作区。稳定模板保留在 `templates/`，当前维护说明保留在 `docs/`；阶段性研究、计划和验证记录归档在 `docs/archive/`。
...
```text
docs/archive/skill-authoring-prespec.md
