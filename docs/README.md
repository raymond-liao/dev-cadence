...
工作项文件使用稳定 ID，常规生命周期由卡片状态与 Backlog 表达，不通过移动文件改变身份。用户明确确认清理已经退出范围或不再需要的历史工作项时，可以删除卡片正文和关联运行记录；仍需保留的终态依据使用 Backlog 紧凑历史行表达。
...
| 目标仓库的 `build/dev-cadence/**` | Delivery Workflow 的 manifest、阶段记录、测试、Review 和验收证据；不属于本源码仓库 |
...
- workflow 运行证据由目标仓库保存在其 `build/dev-cadence/` 下；本源码仓库不使用 `docs/` 复制同一过程事实，也不保留运行记录副本。
