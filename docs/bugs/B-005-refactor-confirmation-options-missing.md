# B-005 已安装 Workflow 用户确认门选项与结果语义不完整
...
修复六个已安装 Workflow 在部分用户确认门只要求“确认”，却没有完整展示实际可选决策及其后果的问题。Refactor 是最先暴露问题的 Workflow，但 Discovery、Work Item Planning、Architecture Design、Feature Dev 和 Bug Fix 也存在同类缺口。

## 预期行为

当已安装 Workflow 到达真实用户决策门时，用户可见摘要必须展示该门实际支持的明确选项，并说明每个选项对下一阶段、资产写入、运行记录、状态和再次确认的影响。不同门禁保留各自业务语义，不强制使用同一套通用选项。
...
1. 六个已安装 Workflow 的每个真实用户决策门都展示实际支持的明确选项及结果。
2. Delivery Workflow 的前置阶段至少明确“确认当前版本并进入下一阶段”和“要求修改并停留当前阶段重提”的行为。
3. 部分确认、方案选择、风险接受、行为路由和 Git 集成保留各自专用语义。
4. Business Acceptance 和 Completion 的现有固定选项不被重复、合并或弱化。
5. 不出现没有记录、状态或后续处理支撑的虚假选项。
6. 普通非终态确认可接受明确自然语言；只有终态、高风险或破坏性操作继续要求编号或精确文本。
7. 契约测试覆盖六个 Workflow，并验证“门、选项、结果”三者一致。
8. source、dist、安装包和版本处理保持一致，完整契约验证通过。
