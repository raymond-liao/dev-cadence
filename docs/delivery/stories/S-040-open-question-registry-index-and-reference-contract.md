...
将 `docs/delivery/open-questions.md` 明确为仓库全部问题的统一 Registry，使用稳定编号、生命周期状态、总表导航和单一正文来源连接各权威资产，同时收紧所有 Dev Cadence 管理文档的快捷链接文本规则。
...
- `docs/delivery/open-questions.md` 是仓库全部 Open Questions 的统一 Registry；PRD、Business Architecture、架构文档、工作项卡片、技术方案和其他权威资产中的每个问题都必须进入该 Registry。
...
- 不在本 Story 中迁移当前 `docs/delivery/open-questions.md` 的格式或编号。
