...
将安装包内的 `git-commit` 定义为 Dev Cadence 内部共享提交能力，并由 `using-dev-cadence` 像路由 Document Conventions 一样统一规定调用边界。只有已经选中或恢复 Dev Cadence Workflow 后才能调用该能力；Dev Cadence 之外的普通提交不得调用。各 Workflow 负责提交时机、范围、精确暂存、检查和证据，`git-commit` 负责检查已暂存提交单元、生成 Conventional Commit 信息并执行一次提交。
...
- 只有 `using-dev-cadence` 已经选中或恢复业务 Workflow，且当前 commit 属于该 Workflow 管理范围时，才允许调用安装包内的 `git-commit`。
- 将该边界覆盖到 Discovery、Work Item Planning、Architecture Design、Feature Dev、Bug Fix 和 Refactor；各 Workflow 继续保留自己的 Asset 或 Delivery 记录模型及提交语义。
- 没有活动 Dev Cadence Workflow 时，普通提交请求按目标仓库的普通 Git 规则处理，不调用安装包内的 `git-commit`。
...
- 修正 `git-commit` 中与内部共享能力冲突或彼此不一致的 Conventional Commit 分类、安全检查和提交后建议。
...
1. `using-dev-cadence` 明确要求先选中或恢复 Workflow，再读取固定路径 `.dev-cadence/skills/git-commit/SKILL.md`。
2. 没有活动 Dev Cadence Workflow 的普通提交请求不会路由到安装包内的 `git-commit`。
3. 六个 Workflow 均可在自身管理的 commit 节点使用共享能力，同时保留各自的提交时机、范围、检查和记录模型。
...
8. `git-commit` 的 Conventional Commit 分类、安全规则和提交后行为内部一致，不输出越过当前 Workflow 的后续建议。
9. 契约检查覆盖入口边界、安装路径、外部禁止调用、子代理传递和安装结果。
10. source、dist、安装包和根版本保持一致，空白检查与完整契约验证通过。
...
- 无。入口集中路由覆盖所有被选中或恢复的 Dev Cadence Workflow；Dev Cadence 之外不得调用，仓库外个人全局 skill 不属于本任务。
