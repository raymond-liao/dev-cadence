...
本文说明产品需求如何从原始产品想法和业务问题逐步形成，并定义 User Journey、业务需求、PRD、Business Architecture、Feature、Story 和 Story Map 之间的关系。
...
- User Journey 描述用户为完成目标而经历的业务流程，每个关键节点代表一个业务需求环节。
- 一个 User Journey 节点可以拆解出一个或多个 Feature。
- 一个 Feature 可以拆解出一个或多个 Story。
...
产品目标、范围、能力、需求     角色、流程、对象、状态、规则
        ...
        +----------+-----------+
            ...
            Work Item Planning
                   ...
                   v
              Story Map
User Journey -> 业务需求 -> Feature -> Story -> 路径与 MVP
```
...
一个产品允许存在多条 User Journey。不同 Journey 可以服务不同角色、目标或场景，也可以共享业务需求、Feature 或 Story。共享内容必须复用稳定身份，不能因为出现在多条 Journey 中就重复创建相互竞争的定义。

## 从 Journey 节点形成业务需求

User Journey 上的每个关键节点代表一个业务需求环节。节点不是页面、技术模块或开发任务，而是用户或业务在该环节必须获得的结果。
...
重要产品需求必须能够追溯到其来源 Journey 和节点。Journey 调整、节点拆分或需求变化时，应能够判断哪些 PRD 内容、Feature 和 Story 受到影响。
...
| 用户价值 | Journey 最终目标和各节点业务结果 |
| 产品范围 | Journey、角色、场景和业务流程边界 |
| 产品能力 | 对多个业务需求的归纳 |
| 产品需求 | Journey 节点产生的业务需求 |
| 成功标准 | Journey 的成功结果和可测量业务结果 |
| 异常和边界需求 | 替代路径与失败路径 |
| Future Scope | 当前未覆盖的 Journey、节点、角色或路径 |
...
PRD 与 Business Architecture 必须保持职责分离，同时能够通过 Journey、业务需求和产品能力建立可追溯关系。
...
User Journey 1:N Journey Node
Journey Node 1:N Feature
Feature 1:N Story
```
...
- 全局 MVP 与后续计划概览；
- 被拆分内容的索引；
- 跨 Journey、Feature 和子文档的关系；
- 全局 Open Questions 和 Change Log。

每个节点只能有一个权威正文位置。根文档和子文档不得重复维护需要同步的完整内容。
...
- Journey、Journey Node 和业务需求的稳定 ID 规则；
- 多条 Journey 共享业务需求或 Feature 时的权威归属；
- PRD Requirement 与 Journey Node 的具体引用格式；
- 全局 Story Map 的最终 Markdown 布局；
- Story Map 达到什么规模时拆分子文档；
- Feature 卡片的最终目录和模板；
- 增量产品设计如何计算 Journey、PRD、Feature 和 Story 的影响范围。
