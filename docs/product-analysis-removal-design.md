...
本文记录当前讨论中已经确认的能力边界。它不是 workflow 执行规则，不替代 `src/workflows/**/SKILL.md`，也不表示相关修改已经开始实施。
...
2. Dev Cadence 必须保留自己的 `docs/backlog.md`。
...
8. 在用户明确授权开始修改前，本方案只用于讨论和设计，不触发 workflow、Backlog、工作项、版本或 Git 变更。
...
- `docs/backlog.md` 汇总工作项并表达统一实施入口、当前生命周期和建议实施顺序。
...
本方案已经确定能力边界，但在实施前仍需基于现有卡片契约完成一次只读盘点，明确删除产品字段后，Story、Task 和 Bug 各自满足“可直接进入实施”的最小字段与成熟度条件。该盘点不得重新引入 Feature、Story Map 或产品分析前置要求。
