...
**This offer MUST be its own message.** Only the offer — no clarifying question, summary, or other content. Wait for the user's response. If they accept, start the server with `--open`; still give them the returned URL as a fallback. If they decline, continue text-only and don't offer again unless they raise it.
