...
REFACTOR_FLOW_DOC="$ROOT_DIR/docs/workflows/refactor.md"
TMP_DIR="$(mktemp -d)"
...
assert_match "refactor flow doc title" "^# 重构流程$" "$REFACTOR_FLOW_DOC"
assert_match "refactor flow doc behavior-preserving definition" "不主动改变外部可观察行为" "$REFACTOR_FLOW_DOC"
assert_match "refactor flow doc requirements stage" "1\\. 需求确认 .*重构需求确认记录" "$REFACTOR_FLOW_DOC"
assert_match "refactor flow doc solution stage" "2\\. 重构方案 .*重构方案" "$REFACTOR_FLOW_DOC"
assert_match "refactor flow doc plan stage" "3\\. 实施计划 .*重构实施计划" "$REFACTOR_FLOW_DOC"
assert_match "refactor flow doc implementation outputs" "4\\. 重构实施 .*可工作的交付物；测试资产；重构记录；代码审查报告" "$REFACTOR_FLOW_DOC"
...
assert_match "refactor flow doc baseline rule" "重构方案必须定义行为基线，回归验证必须使用同一基线证明行为得到保留" "$REFACTOR_FLOW_DOC"
