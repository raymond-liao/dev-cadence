...
assert_workflows "commit after confirmation rule" "After the user confirms a stage output" "After the user confirms a stage output" "After the user confirms a stage output"
...
assert_workflows "no ordinary active run commit" "Do not create an ordinary git commit for unfinished feature work" "Do not create an ordinary git commit for unfinished repair work" "Do not create an ordinary git commit for unfinished refactor work"
assert_workflows "only checkpoint commits during active run" "Only create checkpoint commits for confirmed stage outputs" "Only create checkpoint commits for confirmed stage outputs" "Only create checkpoint commits for confirmed stage outputs"
assert_workflows "active run commit red flags" "### Commit Red Flags" "### Commit Red Flags" "### Commit Red Flags"
assert_workflows "ordinary commit rationalization" "User asked to commit, so ordinary git commit is allowed" "User asked to commit, so ordinary git commit is allowed" "User asked to commit, so ordinary git commit is allowed"
