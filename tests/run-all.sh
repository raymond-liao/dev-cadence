#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

"$ROOT_DIR/tests/package-contract.sh"
bash "$ROOT_DIR/tests/systematic-debugging-find-polluter.sh"
bash "$ROOT_DIR/tests/finishing-a-development-branch-contract.sh"
bash "$ROOT_DIR/tests/finishing-worktree-ownership-contract.sh"
bash "$ROOT_DIR/tests/asset-delivery-record-contract.sh"
bash "$ROOT_DIR/tests/delivery-record-contract.sh"
bash "$ROOT_DIR/tests/feature-persistent-record-recovery-contract.sh"
bash "$ROOT_DIR/tests/architecture-design-contract.sh"
bash "$ROOT_DIR/tests/backlog-contract.sh"
bash "$ROOT_DIR/tests/work-item-analysis-contract.sh"
bash "$ROOT_DIR/tests/change-log-contract.sh"
bash "$ROOT_DIR/tests/document-conventions-contract.sh"
bash "$ROOT_DIR/tests/configuration-contract.sh"
bash "$ROOT_DIR/tests/open-question-registry-contract.sh"
bash "$ROOT_DIR/tests/routing-contract.sh"
bash "$ROOT_DIR/tests/work-item-development-workflow-contract.sh"
bash "$ROOT_DIR/tests/git-commit-contract.sh"
bash "$ROOT_DIR/tests/workflow-symmetry.sh"
bash "$ROOT_DIR/tests/skill-description-contract.sh"
bash "$ROOT_DIR/tests/finishing-discard-contract.sh"
bash "$ROOT_DIR/tests/install-contract.sh"
bash "$ROOT_DIR/tests/whitespace-contract.sh"
bash "$ROOT_DIR/tests/confirmation-gates-contract.sh"
bash "$ROOT_DIR/tests/bug-fix-backlog-sync-contract.sh"
bash "$ROOT_DIR/tests/bug-fix-proof-evidence-contract.sh"
