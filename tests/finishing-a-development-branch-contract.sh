...
assert_literal "normal cleanup branch freeze" 'Immediately before `git worktree remove`, freeze the exact task branch ref and `Expected Current HEAD SHA`' "$FINISHING_SKILL"
assert_literal "normal verifier denial" 'On `not_owned` or any verifier failure, skip `git worktree remove` and preserve the task branch and worktree.' "$FINISHING_SKILL"
assert_literal "no evidence tuple denial" 'With `Created By Current Run: no`, pass the manifest `not_applicable` tuple unchanged; the verifier must reject it.' "$FINISHING_SKILL"
