...
assert_literal "post-confirmation ownership refreeze" 'Immediately after final user confirmation and before any destructive command, freeze the exact task branch ref and `Expected Current HEAD SHA` again and invoke `scripts/verify-worktree-ownership.sh` with the immutable manifest ownership tuple.'
assert_literal "post-confirmation installed verifier path" 'After changing to the target repository root, resolve that verifier as `$MAIN_ROOT/.dev-cadence/vendor/superpowers/skills/finishing-a-development-branch/scripts/verify-worktree-ownership.sh`; do not invoke a source-repository-relative `scripts/...` path.'
assert_literal "post-confirmation ownership denial" 'A `not_owned` result or any verifier failure returns `discard_blocked` before destructive commands and preserves the worktree, task branch, and active run records.'
assert_literal "whole-run no-evidence tuple denial" 'With `Created By Current Run: no`, the manifest `not_applicable` tuple is rejected by the verifier; return `discard_blocked` and preserve the worktree, task branch, and active run records.'
