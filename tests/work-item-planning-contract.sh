...
assert_match "derived refresh no trigger" 'derived.*parallel.*refresh.*must not.*increment.*`Ordering Version`|must not.*increment.*`Ordering Version`.*derived.*parallel.*refresh' "$SKILL"
...
assert_match "partial confirmation remains atomic" 'partial.*confirmation.*must not.*four.*atomic|four.*atomic.*partial.*confirmation.*must not' "$SKILL"
