...
Before entering the finishing flow, rerun the final-verification validator. If it fails, invalidate Business Acceptance evidence as `superseded`, return to Development Implementation, repeat implementation review and System Testing verification, and require Business Acceptance again. When `Final Implementation SHA` is `skipped: no tracked changes`, do not invoke `--final-verification`; preserve the existing terminal validation path.
