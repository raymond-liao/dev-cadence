...
After `Verification End`, only first-parent commits recorded as manifest checkpoint commits that modify only the current run evidence directory are allowed before final revalidation. If the final verification candidate changes, return to Repair Implementation, repeat implementation review, and repeat Regression Verification before Business Acceptance. If `Final Repair SHA` is `skipped: no tracked changes`, do not invoke `--final-verification`; preserve the existing terminal validation path.
...
Before entering the finishing flow, rerun the final-verification validator. If it fails, invalidate Business Acceptance evidence as `superseded`, return to Repair Implementation, repeat implementation review and Regression Verification, and require Business Acceptance again. When `Final Repair SHA` is `skipped: no tracked changes`, do not invoke `--final-verification`; preserve the existing terminal validation path.
