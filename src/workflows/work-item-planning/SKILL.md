...
Estimate Story Map Stories and necessary Tasks relative to the confirmed baseline. Estimate independent Tasks and Bugs only when they are in the explicit estimation scope or will enter an Iteration Plan; Bugs remain outside the Story Map. When information is insufficient, preserve `?`; do not substitute a certain level to complete planning. The Story Map must show the Size for its Stories and necessary Tasks, Path and Milestone distributions, and an explicit summary of `XL`, `?`, and significant uncertainty.
