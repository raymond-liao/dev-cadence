...
After the item is selected and before switching a task branch or creating a worktree, claim it by atomically synchronizing the authoritative card and `docs/backlog.md` row to `In Progress` in the primary checkout (this repository's baseline is `main`) regardless of `worktree.enabled`. A claim uses the current card Version for its Change Log important event and does not increment the card Version for an execution-status-only change. When the status transition is important, append that event according to `.dev-cadence/skills/contracts/change-log.md`; a claim must be idempotent and must not append a duplicate Change Log event. A card already in `In Progress` must not be claimed again in the same request. Before any task workspace is created, write and record a verifiable main claim commit containing the atomic card and Backlog claim; this lifecycle handoff commit is neither a Repair Implementation commit nor a stage checkpoint commit and must not alter existing checkpoint gates.

The workspace preparation must complete before the entry routes downstream. When `worktree.enabled: true`, perform the primary checkout claim write, persist the claim commit, then immediately create or verify the task worktree from that claim commit. When `worktree.enabled: false`, perform the primary checkout claim write, persist the claim commit, then immediately prepare the dedicated task branch from that claim commit and must not create a worktree. Any primary checkout write, claim persist failure, claim commit, workspace baseline, card Version, `In Progress` status, or matching Backlog row verification failure must stop: when `worktree.enabled: true`, it must not create or verify a worktree or route downstream; when `worktree.enabled: false`, it must not prepare a dedicated branch or route downstream. Verify in the selected workspace that the claimed card Version matches the authoritative card Version, along with `In Progress` status and the matching Backlog row, before routing any downstream Delivery Workflow. For B-015, this verification must continue to confirm Version `4`. Do not begin Requirements, Solution, Plan, checkpoint, or implementation work before this handoff completes.
