...
✅ 已确认 B-013 是既有规则错误：Feature 是在存在时应保留的追踪关系，不是所有 Story `Ready` 的通用前置条件。只有 Story 实际需要新建或改变产品级结论时，才应返回 Discovery。诊断范围不包含产品设计资产、Story Map 结构或其他工作项类型的入口规则。
