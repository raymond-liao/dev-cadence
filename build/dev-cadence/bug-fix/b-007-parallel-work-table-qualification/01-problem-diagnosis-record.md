...
- 后续事实：B-009 已验收并移除逐行 `下一步 Workflow / 入口门禁` 列，规定并行表是由待处理顺序派生的四列候选视图。
- 当前权威边界：`状态` 只表示卡片生命周期；Dependency Table 表达依赖；`using-dev-cadence` 和对应 workflow skill 拥有路由与入口门禁。
- 结论：原诊断中的“职责混用”仍成立，但“新增独立入口列”的修复假设已被 B-009 取代；当前缺陷是 B-007 卡片未同步后续权威决定。
