...
B-009 的已验收方案取代“独立入口资格列”：并行表保持 `并行组 | 可并行工作项 | 前置条件 | 状态` 四列，在表级说明中明确状态不表达 Workflow 入口资格，路由由 `using-dev-cadence` 和 owning workflow 负责。

本次只更新 B-007 卡片 Version、预期行为、范围、验收标准和 Open Question，并同步 Backlog Version；不改当前运行时规则、不恢复第五列、不改变排序。
