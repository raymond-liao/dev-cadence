...
当前 Codex Plugin 发布内容包括一个 bootstrap Skill 和一组短前缀工作纪律 Skills：

| 入口 | 用途 |
|---|---|
| `using-dev-cadence` | Dev Cadence Supervisor 入口，负责检查适用 Skills、选择 workflow/task class/gates，并按顺序编排具体 discipline |
| `cadence-clarify` | 澄清目标、范围、非目标、设计、验收和验证方式 |
| `cadence-plan` | 把已确认设计拆成可执行任务和测试计划 |
| `cadence-execute` | 通过 Harness evidence 执行已批准计划 |
| `cadence-tdd` | 对可测试行为变更执行 Red-Green-Refactor |
| `cadence-debug` | 处理 bug、incident、失败测试和未知根因 |
| `cadence-review` | 检查 spec compliance 和 code quality |
| `cadence-verify` | 完成前验证测试、范围、风险和 Human acceptance |
| `cadence-sync` | 初始化、检查、同步或修复薄仓库契约 |

共享治理协议位于 `references/`，任务和 evidence 模板位于 `templates/`，本地校验和辅助工具位于 `scripts/`。
...
打包、注册本地 marketplace 并安装或更新本机 Codex 插件：
