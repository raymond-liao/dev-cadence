...
- 默认不得为用户任务自动选择、启动、恢复或执行任何 Dev Cadence workflow。只有用户明确要求该任务使用 Dev Cadence，或明确点名某个 Dev Cadence workflow 时，才获得该任务范围内的使用授权；仓库中存在 `.dev-cadence`、Backlog、工作项或运行记录，任务语义与 workflow 匹配，以及用户仅提出“实现”“修复”“继续”“提交”等一般动作，都不构成授权。
...
- 实施 `docs/delivery/backlog.md` 中的工作项时，在平台支持且任务边界可独立委派的情况下，优先由子代理执行已确认范围内的实现。
...
- 提交信息必须使用英文，包括 subject 和可选 body。
- 不要 push，除非用户明确要求。
...
- 开始实施 Backlog 工作项前，必须原子地将卡片状态更新为 `In Progress`，并将对应 Backlog 行从 `待处理` 移至 `进行中`；保留无关待处理项的相对顺序，且不得递增 `Ordering Version`。
- 只有在实现已集成且相关验证通过后，才能原子地将卡片状态更新为 `Done`，并将对应 Backlog 行从 `进行中` 移至 `已完成`。
